...
{	
	...
	public void TestDoubleMiles()
	{
		...
		Flight flight2 = new Flight(StartDate, EndDate, 700);
		target.bookWithDoubleMiles(new Booking[]{flight2});
		Assert.assertEquals(700+5000, target.bonusFrequentFlierMiles, 0.01);
	}
	...
	public void TestGetDiscount3(){
		Discount target = new Discount(5, 999999999);
		ServiceLocator.Instance().AddDiscount(target);
		this.target.book(new Booking[]{new Flight(StartDate, EndDate, 100), new Hotel(5), new Car(3)});
		assertEquals(1035,this.target.Price(), 0.01);
